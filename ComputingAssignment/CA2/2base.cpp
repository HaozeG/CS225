...
{
    block = new T*[5];
    for (int i = 0; i < 5; i++) block[i] = NULL;
    number = 0;
    overflow = 0;
    length = 5;
    overblock = 2;
...
template <class T> void Block<T>::insert(T* item)
{
    ...
    else
    {
        ...
        this->number++;
        this->overflow++;
        if (this->overflow == this->overblock) this->sort();
        return;
    }
}
...
template <class T> T* Block<T>::retrieval(const char* id) 
{
    ...
    low = this->overblock; high = this->length - 1;
    while (low <= mid && high >= mid)
    {
        ...
        {cout<<"ha\n";
            int tem = mid;
            if (mid - low == 1) mid = mid - 1;
            else mid = mid - (mid - low) / 2;
            high = tem;
        }
        else
        {
            int tem = mid;
            if (high - mid == 1) mid = mid + 1;
            else mid = mid + (high - mid) / 2;
            low = tem;
        }
    }
    return NULL;
