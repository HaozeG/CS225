...
void Report_system::Month(blist *ptr, long timeoffset)
{
    ...
    do
    {
        for (int relation_index = 0; relation_index < temp->length; relation_index ++)
        {
        //只要进入系统了就算登记过
        keep[0] += 1;
        //已经预约 && 没有治疗 && 没有退出（不过没有用到）
        if((temp->block[relation_index])->registration && !temp->treated && !temp->withdrawn){keep[1] += 1;}
    ...
    if(0 ==keep[3])
    {
        outfile << "Average waiting time: " << "0 days 0 hours." << "\n";
    }
    else
    {
        outfile << "Average waiting time: " << floor( keep[4] / (24 * keep[3]) ) << " days " << (keep[4] / keep[3]) % 24 << " hours." << "\n";
    }
